...
function resultprint = col11(sc, rtc)

a = load('exp-rep-data.csv');
rawRT = load('repRawData');
...
eFile = strcat('sclass.', num2str(sc), '.', num2str(rtc));
fid = fopen(eFile, 'w');
fprintf(fid,'%d\n', SwitchClass);
fclose(fid);

cFile = strcat('nclass.', num2str(sc), '.', num2str(rtc));
fid = fopen(cFile, 'w');
fprintf(fid,'%d\n', NonSwitchClass);
fclose(fid);
